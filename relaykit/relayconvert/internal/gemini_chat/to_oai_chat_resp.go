...
func inlineDataToMarkdown(inlineData *dto.GeminiInlineData, modelVersion string) string {
	if inlineData == nil {
		return ""
	}
	...
	if strings.HasPrefix(mimeType, "image") {
		if pkg.AliyunOssClient != nil {
			...
			dirName := modelVersion
			if dirName == "" {
				dirName = "gemini"
			}
			if imageBytes, err := base64.StdEncoding.DecodeString(data); err == nil {
				if cdnUrl, uploadErr := pkg.AliyunOssClient.UploadFileWithBytes(imageBytes, dirName, fileName); uploadErr == nil {
					return "![image](" + cdnUrl + ")"
				}
			}
		}
	...
	return fmt.Sprintf("[media](data:%s;base64,%s)", mimeType, data)
}
	...
	for _, candidate := range response.Candidates {
		...
		if len(candidate.Content.Parts) > 0 {
			...
			for _, part := range candidate.Content.Parts {
				if part.InlineData != nil {
					writeSep()
					content.WriteString(inlineDataToMarkdown(part.InlineData, response.ModelVersion))
		...
		for _, part := range candidate.Content.Parts {
			if part.InlineData != nil {
				if strings.HasPrefix(part.InlineData.MimeType, "image") {
					writeSep()
					content.WriteString(inlineDataToMarkdown(part.InlineData, geminiResponse.ModelVersion))
				}
			} else if part.FunctionCall != nil {
				isTools = true
				if call := geminiResponseToolCall(&part); call != nil {
